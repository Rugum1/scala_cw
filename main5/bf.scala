    ...
    def jumpRight(prog: String, pc: Int, level: Int) : Int = prog.charAt(pc) match {
        
        case _ if(pc+1 == prog.length) => pc + 1
        case '[' => jumpRight(prog,pc+1,level+1)
        case ']' => if(level  ==  0)  pc + 1 else jumpRight(prog,pc+1,level-1)
        case _ => jumpRight(prog,pc+1,level)
    }

    def jumpLeft(prog: String, pc: Int, level: Int) : Int = prog.charAt(pc) match 
    {            
    ...
    {      
       case prog  => if(Try(prog.charAt(pc)).getOrElse(0) == 0)  mem 
                                    ...
                                    compute(prog, jumpRight(prog, pc + 1, 0), mp, mem) else compute(prog,pc + 1,mp,mem)
